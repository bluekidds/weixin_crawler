...
def get_sentiment(start, end, wanted_word, dbutils, GET_KEYWORDS):
    ...
    for i in range(days+1):
        ...
        if GET_KEYWORDS:
            #all_articles_df = all_articles_df.append(df[df["content"].str.contains(wanted_word)], ignore_index=True) # Sufficient for low volume of data
            for article in articles:
                if wanted_word in article["content"]:
                    keyword_ids.append(article["_id"])
        #else:
        #    all_articles_df.append(df)
        print("Current day:" + str(i) + 'Size is: ' +str(len(articles)))
...
if __name__ == '__main__':
    ...
    print('Working on Sentiment analysis of articles')
